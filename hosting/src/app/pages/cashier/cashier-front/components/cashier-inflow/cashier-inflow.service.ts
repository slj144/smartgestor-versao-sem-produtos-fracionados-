...
import { ICashierInflow, ECashierInflowStatus, ECashierInflowOrigin } from '@shared/interfaces/ICashierInflow';
...
export class CashierFrontInflowService {
  ...
  public async registerInflow(data: ICashierInflow, batch?: IBatch): Promise<void> {
    ...
    return (new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (!data.code) {
          data.code = iTools.FieldValue.control('SystemControls', Utilities.storeID, 'CashierInflows.code');          
          data.owner = Utilities.storeID;
          data.operator = Utilities.operator;
          data.origin = ECashierInflowOrigin.CASHIER;
          ...
          if (!data._id) {
            ...
            docRef = { collName: 'CashierInflows', where: [
  ...
  public async cancelInflow(data: ICashierInflow, batch?: IBatch): Promise<void> {
    ...
    return (new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        batch.update(this.collRef().doc(data._id), {
          status: ECashierInflowStatus.CANCELED
        }, { merge: true });
  ...
  public async deleteInflow(data: ICashierInflow, batch?: IBatch): Promise<void> {
  ...
  private async checkBankAccount(data: ICashierInflow, operation: ('register'|'cancel'), batch: IBatch) {
          ...
          if (account) {
            ...
            const bankTransitions: any = [{
              ...
              transaction: {
                ...
                type: (operation != 'cancel' ? EFinancialBankTransactionType.DEPOSIT : EFinancialBankTransactionType.WITHDRAW),
  ...
  private async systemLogs(data: ICashierInflow, action: string, batch: IBatch, batchRef?: number) {
    ...
    settings.data[0].type = ESystemLogType.CashierInflows;
  ...
  private collRef(): ICollection {
    return this.iToolsService.database().collection('CashierInflows');
  }
