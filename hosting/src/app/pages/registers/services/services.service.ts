...
export class ServicesService {
  ...
  private data: { [_id: string]: IRegistersService } = {};
  ...
  public query(where?: query['where'], reset: boolean = true, flex: boolean = false, scrolling: boolean = false, strict: boolean = true, others?: {back?: boolean, orderBy?: any }) {
    ...
    return (new Promise<IRegistersService[]>((resolve) => {
  ...
  public async registerService(data: IRegistersService, batch?: IBatch) {
    ...
    return (new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        if (!Utilities.isMatrix){
          ...
          data.branches[Utilities.storeID] = data.branches[Utilities.storeID] || {};

          if (data.costPrice != undefined){
            data.branches[Utilities.storeID].costPrice = data.costPrice;
          }

          if (data.executionPrice != undefined){
            data.branches[Utilities.storeID].executionPrice = data.executionPrice;
          }


          delete data.costPrice;
          delete data.executionPrice;
        }
  ...
  public async deleteService(data: IRegistersService, batch?: IBatch) {
  ...
  private async systemLogs(data: IRegistersService, action: string, batch: IBatch, batchRef?: number) {
  ...
  private treatData(id: string, data?: IRegistersService[], order: 1 | -1 = 1) {
