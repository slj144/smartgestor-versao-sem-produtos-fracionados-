...
// Translate
import { ProductDepartmentsTranslate } from "./product-departments.translate";

// Interfaces
import { IBatch } from '@itools/interfaces/IBatch';
import { ICollection } from '@itools/interfaces/ICollection';
import { IStockProductDepartment } from '@shared/interfaces/IStockProductDepartment';
...
export class ProductDepartmentsService {
  ...
  public translate = ProductDepartmentsTranslate.get();
  ...
  public getDepartments(listenerId: string, listener: ((_: any) => void)) {
  ...
  public async registerDepartment(data: IStockProductDepartment, batch?: IBatch): Promise<void> {
  ...
  public async deleteDepartment(data: IStockProductDepartment, batch?: IBatch): Promise<void> {
  ...
  private async systemLogs(data: IStockProductDepartment, action: string, batch: IBatch, batchRef?: number) {
    ...
    settings.data[0].type = ESystemLogType.StockProductDepartments;
  ...
  private collRef(): ICollection {
    return this.iToolsService.database().collection('StockProductDepartments');
  }
  ...
  private requestData() {
    ...
    this.collRef().limit(100).onSnapshot((response) => {
